...
install:
	$(info $(pink)------------------------------------------------------)
	$(info $(pink)Make ($(os)): Installing Tintol Sample Topup Project...)
	$(info $(pink)------------------------------------------------------$(reset))
...
update:
	$(info $(pink)------------------------------------------------------)
	$(info $(pink)Make ($(os)): Updating Tintol Sample Topup Project...)
	$(info $(pink)------------------------------------------------------$(reset))
	@docker-compose down
	@docker-compose build
	@docker-compose up -d
	...
	@make -s start
start:
	$(info $(pink) Make ($(os)): Starting Tintol Sample Topup Project.)
	@chmod -R 0777 storage
	@docker-compose up -d
	@docker exec -it rstp_php service cron start
	@docker exec -it rstp_php chmod -R 0777 storage
stop:
	$(info $(pink) Make ($(os)): Stopping Tintol Sample Topup Project.)
	@docker-compose down
restart:
	$(info $(pink)Make ($(os)): Restarting Tintol Sample Topup Project.)
	@make -s stop
	@make -s start
console:
	$(info $(pink)Make ($(os)): Starting Console for Tintol Sample Topup Project.)
